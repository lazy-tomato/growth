...
- 前面一直提到的 Watcher ，现在来基本的了解了解 Watcher。
...
- `mountComponent`中 1.`new Watcher`;2. 传入了`updateComponent`一个箭头函数。
...
此时有两个 Watcher，一个是渲染整个页面的 watcher。一个是通过 watch 配置的一个 watcher。

1. 渲染 watcher 传入的 expOrFn 是一个箭头函数
...
简单来说，value 就是我们 data + 表达式的值，如果 deep 配置存在，则深层监听表达式对应的值，通过深层遍历，每个属性都访问一遍，就能够达到收集依赖的目的。
