...
export default class Watcher {
  ...
  update() {
    ...
    if (this.lazy) {
      ...
      // 主要是执行 queueWatcher, !! 这里就是 watcher 异步更新队列的逻辑！
      queueWatcher(this)
    }
  }
...
1. 整个 Watcher 就是一个类，当然它的实例上有很多属性，用来做不同的逻辑处理。

2. 当我们渲染组件的时候 就会 `new Watcher()`。

   > 当然 我暂时已知的除了渲染的时候会 `new Watcher()`， 我们定义组件的 `watch/computed` 选项的时候，也会`new Watcher()`来处理相关逻辑。
   > 暂时就按照 渲染 `Watcher` 来理解。

3. `new Watcher()` 就会执行 `this.get()`.

   - `Dep.target = 当前的 watcher` 在 Dep 类上的 target 属性上绑定当前 watcher。
   - 然后触发 `vm._render()` 函数 ;
   - `vm._render()` 又会读取数据，触发数据的 get。

   > `vm._render()` 后续再讲，暂时简单理解，就是一个函数。
...
- 此时 Dep.target 上存储的是我们当前的 watcher 实例。

- `dep.depend()` => `Dep.target.addDep(this);` => 其实就是在 当前 watcher 中存储当前的 dep; 在当前 dep 中存储当前的 watcher。

5. 在回顾一下数据的 set

- 当我们修改数据的时候，会触发 对应的 set。
- set 会触发 `dep.notify()`；
- `notify()`会遍历 dep 中存储 `watcher` 的数组 `subs`,依次执行 `watcher` 的 `update()`;
- 最后会执行 `run`。 （当然还有可能执行 `queueWatcher` ）
